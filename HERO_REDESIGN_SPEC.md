...
The scene was rebuilt in RAW three.js (no @react-three/fiber — R3F v8 crashes on Next 16 / React 19) and it works:
...
- **Blowout already fixed:** the voxel material is `THREE.NormalBlending` (NOT Additive — additive blew the center to pure white). Keep Normal.
...
1. **Phone reveal is BURIED.** On scroll the phone rises (`phone.position.y` lerp in `poseScene`) but lands behind/inside the ball and is too dark (dark-mode body lerps to steel) → invisible. Rework per the NEW narrative below — the phone must be lit, pulled forward in z, and land in clear center stage.
...
3. **Light mode unverified** — test BOTH light (cream `--color-canvas`) and dark (steel) via the header toggle.
...
3. Claude Preview: `preview_start "buzzr-web"` (`.claude/launch.json`, :3010). Screenshot light AND dark.
